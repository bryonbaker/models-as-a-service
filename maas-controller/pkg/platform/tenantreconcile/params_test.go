...
func TestApplyPlatformParamsWithRenderedOverlay(t *testing.T) {
	...
	for i := 2; i < 4; i++ {
		...
		disabledPost, foundPost, err := unstructured.NestedBool(cp, "patch", "value", "typed_per_filter_config", "envoy.filters.http.ext_proc.ipp", "disabled")
		require.NoError(t, err, "configPatches[%d] ipp disabled field", i)
		require.True(t, foundPost, "configPatches[%d] ipp disabled field should exist", i)
		assert.True(t, disabledPost, "configPatches[%d] ipp should be disabled", i)
	}
